...
この確認中に、練習の進行指示が返答本文へ漏れる不具合を見つけました（3往復目の返答末尾に「（※残り2往復です。自然なやり取りを続けましょう）」が付いた）。`chatInstructions` に往復数を返答へ書かない旨を追記し、`test/practice.test.ts` で全往復について指示の存在を確認しています。修正の効果は実測できていません。修正後に48件（4シナリオ×2〜4往復目×4回）を採取して漏れ0件でしたが、旧プロンプトへ一時的に戻した同条件の対照48件でも0件で、比較が成立しませんでした。定型文を並べた合成履歴では元の症状が再現しないためです。自然な会話で生成した返答を数えると約97件に1件（約1%）で、この頻度なら修正が無効でも48件で0件になる確率が6割程度あります。95%の確度には約300件の採取が必要ですが、返答末尾に括弧書きが1つ付くだけの表示上の症状であり、追記した1文とテストによる文言確認で足りると判断して、実利用での観察に委ねます。再発を見かけたら、合成履歴ではなく実際に生成した会話で採取してください。
