...
2026-09-10 19:02 JST に4シナリオ対応をバージョン `d69807bf-66a8-4933-a402-e25100606a1e` で反映しました。トラフィックは100%切り替え済みで、既存の Secret を保持しています。`late-report` に加えて `mistake-report`（佐藤）・`low-motivation`（鈴木）・`attitude-issue`（山本）を選べます。往復数が返答本文へ漏れる不具合の修正も含みます。最新版のアプリを更新インストールしてください。

デプロイ後はヘルスチェック200、認証なしのチャット401、未知の `scenarioId` は AI を呼ばずに400を確認しました。加えて4シナリオそれぞれで Gemini を1往復ずつ実際に呼び出し、4人が別人として応答すること、`provider` と `practice` が返ることを確認しています。過去のデプロイ確認と異なり、この確認では実際の AI を呼び出しています。

2026-09-09 22:41 JST に5往復の練習・採点 API をバージョン `b060487a-c33c-4b25-a3c9-69f3b4cde948` で反映しました。既存の Secret を保持しています。
...
シナリオ選択・AI 選択・5往復の練習・採点に対応したバックエンドをデプロイ済みです。以下の接続設定を使い、最新版アプリの会話開始画面でシナリオと OpenAI / Gemini を選択してください。
